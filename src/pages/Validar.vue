<template>
  <q-page class="q-pa-md row items-start q-gutter-md">
    <q-card flat bordered class="my-card">
      <q-card-section class="bg-primary">
        <div class="row items-center no-wrap">
          <div class="col ">
            <div class="text-h6 text-white">CFGS</div>
          </div>

          <div class="col-auto">
            <q-btn color="white" round flat icon="more_vert" aria-label="submenu">
              <q-menu cover auto-close
                      transition-show="flip-right"
                      transition-hide="flip-left">
                <q-list>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Todos</q-btn></q-item-section>
                  </q-item>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Ninguno</q-btn></q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
          </div>
        </div>
        <q-separator/>
      </q-card-section>

      <q-card-section>
        <q-list bordered separator class="q-mt-md bg-grey-1">
          <q-item>
            <q-item-section>
              Marcos Valens Fraile
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Julien Gillet
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Joan Cabot
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>
        </q-list>
      </q-card-section>
    </q-card>
    <q-card flat bordered class="my-card">
      <q-card-section class="bg-primary">
        <div class="row items-center no-wrap">
          <div class="col ">
            <div class="text-h6 text-white">CFGS</div>
          </div>

          <div class="col-auto">
            <q-btn color="white" round flat icon="more_vert" aria-label="submenu">
              <q-menu cover auto-close
                      transition-show="flip-right"
                      transition-hide="flip-left">
                <q-list>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Todos</q-btn></q-item-section>
                  </q-item>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Ninguno</q-btn></q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
          </div>
        </div>
        <q-separator/>
      </q-card-section>

      <q-card-section>
        <q-list bordered separator class="q-mt-md bg-grey-1">
          <q-item>
            <q-item-section>
              Marcos Valens Fraile
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Julien Gillet
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Joan Cabot
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>
        </q-list>
      </q-card-section>
    </q-card>
    <q-card flat bordered class="my-card">
      <q-card-section class="bg-primary">
        <div class="row items-center no-wrap">
          <div class="col ">
            <div class="text-h6 text-white">CFGS</div>
          </div>

          <div class="col-auto">
            <q-btn color="white" round flat icon="more_vert" aria-label="submenu">
              <q-menu cover auto-close
                      transition-show="flip-right"
                      transition-hide="flip-left">
                <q-list>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Todos</q-btn></q-item-section>
                  </q-item>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Ninguno</q-btn></q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
          </div>
        </div>
        <q-separator/>
      </q-card-section>

      <q-card-section>
        <q-list bordered separator class="q-mt-md bg-grey-1">
          <q-item>
            <q-item-section>
              Marcos Valens Fraile
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Julien Gillet
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Joan Cabot
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>
        </q-list>
      </q-card-section>
    </q-card>
    <q-card flat bordered class="my-card">
      <q-card-section class="bg-primary">
        <div class="row items-center no-wrap">
          <div class="col ">
            <div class="text-h6 text-white">CFGS</div>
          </div>

          <div class="col-auto">
            <q-btn color="white" round flat icon="more_vert" aria-label="submenu">
              <q-menu cover auto-close
                      transition-show="flip-right"
                      transition-hide="flip-left">
                <q-list>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Todos</q-btn></q-item-section>
                  </q-item>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Ninguno</q-btn></q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
          </div>
        </div>
        <q-separator/>
      </q-card-section>

      <q-card-section>
        <q-list bordered separator class="q-mt-md bg-grey-1">
          <q-item>
            <q-item-section>
              Marcos Valens Fraile
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Julien Gillet
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Joan Cabot
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>
        </q-list>
      </q-card-section>
    </q-card>
    <q-card flat bordered class="my-card">
      <q-card-section class="bg-primary">
        <div class="row items-center no-wrap">
          <div class="col ">
            <div class="text-h6 text-white">CFGS</div>
          </div>

          <div class="col-auto">
            <q-btn color="white" round flat icon="more_vert" aria-label="submenu">
              <q-menu cover auto-close
                      transition-show="flip-right"
                      transition-hide="flip-left">
                <q-list>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Todos</q-btn></q-item-section>
                  </q-item>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Ninguno</q-btn></q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
          </div>
        </div>
        <q-separator/>
      </q-card-section>

      <q-card-section>
        <q-list bordered separator class="q-mt-md bg-grey-1">
          <q-item>
            <q-item-section>
              Marcos Valens Fraile
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Julien Gillet
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Joan Cabot
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>
        </q-list>
      </q-card-section>
    </q-card>
    <q-card flat bordered class="my-card">
      <q-card-section class="bg-primary">
        <div class="row items-center no-wrap">
          <div class="col ">
            <div class="text-h6 text-white">CFGS</div>
          </div>

          <div class="col-auto">
            <q-btn color="white" round flat icon="more_vert" aria-label="submenu">
              <q-menu cover auto-close
                      transition-show="flip-right"
                      transition-hide="flip-left">
                <q-list>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Todos</q-btn></q-item-section>
                  </q-item>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Ninguno</q-btn></q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
          </div>
        </div>
        <q-separator/>
      </q-card-section>

      <q-card-section>
        <q-list bordered separator class="q-mt-md bg-grey-1">
          <q-item>
            <q-item-section>
              Marcos Valens Fraile
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Julien Gillet
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Joan Cabot
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>
        </q-list>
      </q-card-section>
    </q-card>
    <q-card flat bordered class="my-card">
      <q-card-section class="bg-primary">
        <div class="row items-center no-wrap">
          <div class="col ">
            <div class="text-h6 text-white">CFGS</div>
          </div>

          <div class="col-auto">
            <q-btn color="white" round flat icon="more_vert" aria-label="submenu">
              <q-menu cover auto-close
                      transition-show="flip-right"
                      transition-hide="flip-left">
                <q-list>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Todos</q-btn></q-item-section>
                  </q-item>
                  <q-item>
                    <q-item-section><q-btn class="bg-primary text-white" aria-label="todos">Ninguno</q-btn></q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
          </div>
        </div>
        <q-separator/>
      </q-card-section>

      <q-card-section>
        <q-list bordered separator class="q-mt-md bg-grey-1">
          <q-item>
            <q-item-section>
              Marcos Valens Fraile
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Julien Gillet
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>

          <q-item>
            <q-item-section>
              Joan Cabot
            </q-item-section>
            <q-btn flat class="bg-primary text-white q-mx-sm" aria-label="validar">Validar</q-btn>
            <q-btn flat class="bg-primary text-white" aria-label="cancelar">Cancelar</q-btn>
          </q-item>
        </q-list>
      </q-card-section>
    </q-card>
  </q-page>
</template>

<script>
  export default {
    name: "Validar",
    data() {
      return {}
    }
  }
</script>

<style lang="stylus" scoped>
  .my-card
    width 100%
    max-width 450px
</style>
